...
| Продление Support | Реализовано, без оплаты | Менеджер с отдельным правом запрашивает 7/14/30 дней только для работника из своей области доступа. Окончательное решение принимает только владелец фирмы; одобрение создаёт новый период после действующего срока, а запрос и решение сохраняются в audit log. Store billing и receipt validation не входят в этот сценарий. |
