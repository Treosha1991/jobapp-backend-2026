...
| Задачи | Основа API/mobile | Отдельный законченный web-workflow пока отсутствует. |
| Объявления | Реализовано | Web-черновик, отдельная публикация/снятие с публикации, обязательный текст RU/EN/PL/UK и получатели по проекту, экипажу, дому, вручную выбранным работникам или всему текущему списку доступных работников. |
| Audit log | Реализовано | Employer web viewer с фильтрами по проектам, экипажам, жилью, графику, автопарку, правам, объявлениям и другим действиям. Владелец/управляющий организацией видит историю фирмы; отдельный менеджер — только собственные действия. |
| Продление Support | Реализовано, без оплаты | Менеджер с отдельным правом запрашивает 7/14/30 дней только для работника из своей области доступа, а владелец одобряет или отклоняет. Владелец также может сразу продлить доступ работнику без запроса самому себе. Оба пути создают период после действующего срока и сохраняются в audit log. Store billing и receipt validation не входят в этот сценарий. |
