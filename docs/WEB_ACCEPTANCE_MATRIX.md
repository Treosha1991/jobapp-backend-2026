...
| P1 | Продление Support по запросу менеджера | AUTO PASS | Менеджер может запросить 7/14/30 дней только для работника из своей области; сам подтвердить запрос не может. Только владелец принимает решение, а одобрение создаёт следующий период доступа и два audit event. Это не Store billing. |
| P2 | Задачи работодателя | PARTIAL | API/mobile foundation существует; завершённого web-workflow и полной приёмки нет. |
| P2 | Объявления работодателя | AUTO PASS | Web-черновик, отдельная публикация/снятие с публикации и получатели по проекту, экипажу, дому или выбранным работникам покрыты серверным тестом. |
