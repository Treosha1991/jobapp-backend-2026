...
| P1 | Продление Support | AUTO PASS | Менеджер может запросить 7/14/30 дней только для работника из своей области; сам подтвердить запрос не может. Владелец решает его запросы либо сразу продлевает доступ работнику; оба пути создают следующий период и audit event. Это не Store billing. |
| P2 | Задачи работодателя | PARTIAL | API/mobile foundation существует; завершённого web-workflow и полной приёмки нет. |
| P2 | Объявления работодателя | AUTO PASS | Web-черновик, отдельная публикация/снятие с публикации и получатели по проекту, экипажу, дому, выбранным работникам или всему текущему списку доступа. Контрол выбора покрыт server-rendered тестом; выбор в браузере не обходит серверную проверку worker scope. |
| P2 | Просмотр audit log работодателем | PARTIAL | Реализован web viewer с безопасным разделением всей истории фирмы и собственных действий менеджера. Retention и экспорт истории отдельно не входят в текущий MVP. |
