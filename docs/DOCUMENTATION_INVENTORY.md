...
| `19-subscription-status-and-sponsored-access.md` | `PARTIAL` | Ручной grant JobHub, запрос менеджера с решением владельца и прямое продление владельцем на 7/14/30 дней реализованы в employer web workspace без оплаты. Apple/Google billing, receipt validation, sponsored access и отдельный mobile UI для запроса остаются `PLANNED`. |
