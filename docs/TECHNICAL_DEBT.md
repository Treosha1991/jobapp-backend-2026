...
- **Состояние:** работник вносит часы после начала смены и редактирует их до
...
### TD-015 — Нужна юридическая приёмка хранения обычных изображений чата (`P1`)

- **Область:** приватные фотографии/скриншоты в рабочих чатах.
- **Состояние:** реализовано отдельное закрытое R2-хранилище, очистка EXIF,
  временные подписанные URL, tenant/conversation checks и удаление через 30 дней
  после удаления последней ссылки. Документы через чат по-прежнему запрещены.
- **Риск:** техническое правило хранения не заменяет privacy policy, DPA и
  согласованные права субъекта данных до публичного запуска.
- **Критерии закрытия:** юридически утвердить тексты, основания обработки,
  экспорт/удаление данных и регион хранения; настроить ежедневный production
  job `purge_deleted_support_chat_images` и проверить lifecycle на staging.

## Закрытые пункты

### TD-014 — Worker Today показывает одну основную смену дня (`закрыт 31.08.2026`)

- **Результат:** мобильная модель обратно совместимо принимает `shifts` и
  одиночный `shift`; страница «Сегодня» показывает отдельную компактную карточку
  каждой смены выбранной даты.
- **Привязка действий:** список экипажа и открытие peer-чата используют UUID
  конкретной смены. Запрос и фактические часы остаются общими для даты согласно
  текущей day-level модели `WorkerRequest`/`WorkTimeEntry`.
- **Проверка:** модель покрыта тестом двух смен и legacy payload; окончательная
  визуальная проверка длинных составов остаётся частью TestFlight QA.
